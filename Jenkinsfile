pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/akshaydivekar0624/Ekart-project.git'
            }
        }
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t akshaydivekar0624/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u akshaydivekar0624 -p ${dockerhubpwd}'}
                   sh 'docker push akshaydivekar0624/ekart:latest'
                }
            }
        }
